...
export const CreateStatus: React.FC = () => {
	...
	const validateNewStatus = async (
		type: keyof typeof StatusTypes,
		data: any,
	): Promise<void> => {
		setValid(false);

		const res = await request<{ guild: GuildInvite }>(
			'post',
			'/status/validate',
			{
				data: { type, data },
									...
									<Block flex={0} row hCenter>
										...
										{eventDetails?.endDate && (
											<Text marginLeft={10} marginRight={5} color={theme.text}>
												{dayjs(eventDetails.endDate).format(
													'DD/MM/YYYY - HH:MM',
												)}
											</Text>
										)}
	...
	return (
		<Block flex={0} marginRight={11} hCenter>
			<Block
				...
				color={
					StatusColors[theme.name][statusType][
						selectedType === statusType ? 'color' : 'backColor'
					]
				}
			/>
			<Block
